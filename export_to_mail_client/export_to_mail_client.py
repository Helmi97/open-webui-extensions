"""
title: Export to Mail Client
author: Helmi Chaouachi
git_url: https://github.com/Helmi97/open-webui-extensions/tree/main/export_to_mail_client
version: 1.0.0
required_open_webui_version: 0.5.0
description: Opens the user's default mail client with the current assistant message.
icon_url: https://www.svgrepo.com/show/295339/email-laptop.svg
"""
...
class Action:
    ...
    def _debug_log(self, message: str, **context: Any) -> None:
        ...
        if not context:
            LOGGER.info("[export_to_mail_client] %s", message)
            return
    ...
    def _build_mail_client_url(
        self,
        recipients: EmailRecipients,
        draft: EmailDraft,
    ) -> str:
        self._debug_log(
            "Building mail client compose URL",
            recipient=recipients.to,
            cc=recipients.cc,
            bcc=recipients.bcc,
            subject=draft.subject,
        ...
        return self._build_mailto_url(recipients, draft)

    def _build_mail_client_execute_code(
        self,
        mail_client_url: str,
        return_result: bool,
    ) -> str:
        success_result = (
            "return { ok: true, method: 'mailto_handler', url };"
            if return_result
            else "return;"
        )
        error_result = (
            "return { ok: false, method: 'mailto_handler_error', url };"
            if return_result
            else "return;"
        )
        ...
        return "".join(
            (
                f"const url = {json.dumps(mail_client_url)};",
                "let anchor = null;",
                "try {",
                ...
                f"  {success_result}",
                "} catch (e) {",
                "  try { if (anchor) { anchor.remove(); } } catch (ignored) {}",
                f"  {error_result}",
                "}",
            )
        )

    async def _open_mail_client(
        self,
        mail_client_url: str,
        __event_call__,
        __event_emitter__,
        ...
        if __event_call__ is not None:
            self._debug_log("Opening mail client via __event_call__ execute")
            result = await __event_call__(
                {
                    "type": "execute",
                    "data": {
                        "code": self._build_mail_client_execute_code(
                            mail_client_url,
                            return_result=True,
                        ),
                    },
            ...
            await __event_emitter__(
                {
                    ...
                    "data": {
                        "code": (
                            f"(() => {{ {self._build_mail_client_execute_code(mail_client_url, return_result=False)} }})();"
                        ),
                    },
                }
            )
    ...
    async def _handle_success(self, __event_emitter__) -> None:
        ...
        self._debug_log("Action completed successfully", client="Mail Client")
    ...
    async def _handle_failure(
        ...
        if self.valves.debug:
            LOGGER.exception("[export_to_mail_client] Action failed")
        else:
            LOGGER.error("[export_to_mail_client] Action failed: %s", exc)
    ...
    async def action(
        ...
        try:
            ...
            mail_client_url = self._build_mail_client_url(recipients, draft)
            self._debug_log(
                "Mail client compose target resolved",
                url_length=len(mail_client_url),
                url_preview=self._preview(mail_client_url, 300),
            )

            execute_result = await self._open_mail_client(
                mail_client_url,
                __event_call__,
                __event_emitter__,
            )
            ...
            if isinstance(execute_result, dict) and execute_result.get("ok") is False:
                raise ValueError("The browser could not launch the mail client.")
