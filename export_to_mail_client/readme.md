# Export to Mail Client

Open WebUI action that opens the user's default mail client for the latest assistant message.
...
The action launches the system mail handler using `mailto:`.
...
- Opens the user's default mail client using a `mailto:` draft URL.
...
- The action uses the browser to invoke the system `mailto:` handler.
- If the user wants desktop Outlook specifically, Outlook must be the registered `mailto:` handler in the operating system.

## Installation

1. Open Open WebUI as an admin.
2. Go to the Functions / Actions area.
3. Create or update an Action function with the contents of `export_to_mail_client.py`.
...
5. The browser triggers the system mail handler with the generated draft.
6. After the browser execute step succeeds, the success notification is shown.

## Notifications

- Loading notification: emitted after the last prompt is completed and before the mail-client launch runs.
- Success notification: emitted after the browser execute step succeeds.
- Error notification: emitted if the launch step itself fails.
...
| `loading_notification_text` | `"Generating the email draft..."` | Loading notification shown before the launch step. |
| `success_notification_text` | `"The email is being generated and will be shown soon."` | Success notification shown after the launch step succeeds. |
...
The action builds a `mailto:` URL and triggers it through browser-side `execute` code.

If the execute step throws in the `__event_call__` path, the action raises:

```text
The browser could not launch the mail client.
```

If Open WebUI only provides `__event_emitter__` and not `__event_call__`, the action still attempts the launch but cannot observe a return value.
...
- This action opens the user's mail client. It does not send the email.
- The final body is plain text, not rich HTML.
- The action depends on browser-side execute support from Open WebUI.
- Browser and OS behavior for `mailto:` varies.
- The action cannot reliably verify that the external mail client actually opened a compose window.

## Files

- Action source: `export_to_mail_client.py`
- README: `readme.md`
