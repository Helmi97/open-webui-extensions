"""
title: Export to Outlook Web
author: Helmi Chaouachi
git_url: https://github.com/Helmi97/open-webui-extensions/tree/main/export_to_outlook_web
version: 1.0.0
required_open_webui_version: 0.5.0
description: Opens an Outlook Web compose screen with the current assistant message.
icon_url: https://www.svgrepo.com/show/452067/ms-outlook.svg
"""
...
OUTLOOK_WEB_POPUP_NAME = "outlook_web_compose_popup"
OUTLOOK_WEB_POPUP_FEATURES = (
...
class Action:
    class Valves(BaseModel):
        ...
        outlook_base_url: str = Field(
            default="https://outlook.office.com/mail/deeplink/compose",
            description=(
                "Outlook Web compose URL. Change this to "
                "https://outlook.live.com/mail/0/deeplink/compose for personal "
                "Outlook.com, Hotmail, or Live accounts if needed."
            ),
        )
    ...
    def _debug_log(self, message: str, **context: Any) -> None:
        ...
        if not context:
            LOGGER.info("[export_to_outlook_web] %s", message)
            return
    ...
    def _build_outlook_web_base_url(self) -> str:
        base_url = (
            self.valves.outlook_base_url.strip()
            or "https://outlook.office.com/mail/deeplink/compose"
        )
        cleaned_base_url = base_url.rstrip("?")
        self._debug_log(
            "Resolved Outlook Web compose base URL",
            outlook_base_url=cleaned_base_url,
        )
        return cleaned_base_url

    def _build_outlook_web_url(
        self,
        recipients: EmailRecipients,
        draft: EmailDraft,
    ) -> str:
        self._debug_log(
            "Building Outlook Web compose URL",
            recipient=recipients.to,
            cc=recipients.cc,
            bcc=recipients.bcc,
            subject=draft.subject,
            body_length=len(draft.body),
            outlook_base_url=self.valves.outlook_base_url.strip(),
        )

        return self._build_webmail_url(
            self._build_outlook_web_base_url(),
            {
                "to": recipients.to,
                "cc": recipients.cc,
                "bcc": recipients.bcc,
    ...
    def _build_outlook_web_execute_code(
        self,
        outlook_web_url: str,
        return_result: bool,
    ) -> str:
        ...
        return "".join(
            (
                f"const url = {json.dumps(outlook_web_url)};",
                "let popupWindow = null;",
                "try {",
                f"  popupWindow = window.open(url, '{OUTLOOK_WEB_POPUP_NAME}', '{OUTLOOK_WEB_POPUP_FEATURES}');",
                "} catch (e) {}",
    ...
    async def _open_outlook_web_compose(
        self,
        outlook_web_url: str,
        __event_call__,
        __event_emitter__,
        ...
        if __event_call__ is not None:
            self._debug_log("Opening Outlook Web via __event_call__ execute")
            result = await __event_call__(
                {
                    "type": "execute",
                    "data": {
                        "code": self._build_outlook_web_execute_code(
                            outlook_web_url,
                            return_result=True,
                        ),
                    },
            ...
            await __event_emitter__(
                {
                    ...
                    "data": {
                        "code": (
                            f"(() => {{ {self._build_outlook_web_execute_code(outlook_web_url, return_result=False)} }})();"
                        ),
                    },
                }
            )
    ...
    async def _handle_success(self, __event_emitter__) -> None:
        ...
        self._debug_log("Action completed successfully", client="Outlook Web")
    ...
    async def _handle_failure(
        ...
        if self.valves.debug:
            LOGGER.exception("[export_to_outlook_web] Action failed")
        else:
            LOGGER.error("[export_to_outlook_web] Action failed: %s", exc)
    ...
    async def action(
        ...
        try:
            self._debug_log(
                ...
                outlook_base_url=self.valves.outlook_base_url,
            )
            ...
            outlook_web_url = self._build_outlook_web_url(recipients, draft)
            self._debug_log(
                "Outlook Web compose target resolved",
                url_length=len(outlook_web_url),
                url_preview=self._preview(outlook_web_url, 300),
            )

            execute_result = await self._open_outlook_web_compose(
                outlook_web_url,
                __event_call__,
                __event_emitter__,
            )
