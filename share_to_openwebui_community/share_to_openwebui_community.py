"""
title: Share to OpenWebUI Community
author: Helmi Chaouachi
git_url: https://github.com/Helmi97/open-webui-extensions/tree/main/share_to_openwebui_community
version: 1.0.0
required_open_webui_version: 0.5.0
description: Opens the OpenWebUI Community post composer with content from the current assistant message.
icon_url: https://openwebui.com/favicon.png
"""
...
COMMUNITY_POPUP_NAME = "openwebui_community_share_popup"
COMMUNITY_POPUP_FEATURES = (
...
class Action:
    class Valves(BaseModel):
        ...
        community_post_base_url: str = Field(
            default="https://openwebui.com/post",
            description="Base compose URL for OpenWebUI Community posts.",
        )
        default_title: str = Field(
            default="Assistant message",
            description="Fallback title used when no title can be derived.",
        )
        use_llm_share_generation: bool = Field(
            default=True,
            title="Use Share Generation",
            description=(
                "Use the LLM to generate Community post fields. When disabled, "
                "the content field is prefilled from the assistant message and the "
                "title falls back to the first non-empty line."
            ),
        )
        share_model: str = Field(
            default="",
            title="Share Generation Model",
            description="Optional model ID for Community share generation.",
        )
        share_generation_temperature: float = Field(
            ...
            description="Temperature used for Community share generation.",
        )
        share_generation_max_tokens: int = Field(
            ...
            description="Maximum tokens used for Community share generation.",
        )
        share_generation_prompt: str = Field(
            default=(
                "You are preparing an OpenWebUI Community post draft from an "
                "assistant message. Return only valid JSON in this exact shape: "
                '{"title":"<short plain-text post title>","content":"<markdown post content>"}. '
                "Preserve Markdown formatting in content when useful, including "
                "headings, emphasis, lists, blockquotes, links, and code fences. "
                "Do not wrap the JSON in markdown fences. The title should be "
                "concise plain text."
            ),
            title="Share Generation Prompt",
            description="System prompt used for Community share generation.",
        )
        loading_notification_text: str = Field(
            default="Preparing the OpenWebUI Community post draft...",
        ...
        success_notification_text: str = Field(
            default="The OpenWebUI Community post composer is opening.",
        ...
        max_title_chars: int = Field(
            ...
            ge=1,
            le=300,
            description="Maximum number of characters placed into the title field.",
        )
        max_content_chars: int = Field(
            default=5000,
            ge=1,
            le=20000,
            description="Maximum number of characters placed into the content field.",
        )
    ...
    def _debug_log(self, message: str, **context: Any) -> None:
        if not self.valves.debug:
            return
        if not context:
            LOGGER.info("[share_to_openwebui_community] %s", message)
            return
        rendered = []
        ...
        LOGGER.info(
            "[share_to_openwebui_community] %s | %s",
            message,
            "; ".join(rendered),
        )

    async def _emit_notification(
        self, __event_emitter__, notification_type: str, content: str
    ) -> None:
        ...
        await __event_emitter__(
            {
                "type": "notification",
                "data": {"type": notification_type, "content": content},
            }
        )
    ...
    def _normalize_line(self, raw_line: str) -> str:
        ...
        return HORIZONTAL_WHITESPACE_RE.sub(
            " ", stripped.strip(" \"'`*_~")
        ).strip()
    ...
    def _normalize_title(self, value: Any) -> str:
        raw = self._normalize_text(self._extract_text(value))
        if not raw:
            return ""
        first_line = next((line for line in raw.splitlines() if line.strip()), "")
        return WHITESPACE_RE.sub(" ", first_line).strip()
    ...
    def _extract_share_content_from_text(self, text: str) -> dict[str, str]:
        ...
        for candidate in candidates:
            ...
            normalized = {
                ...
                "content": self._normalize_markdown_content(
                    self._extract_text(parsed.get("content"))
                ),
            }
    ...
    def _resolve_model_id(self, body: dict, __model__) -> str:
    ...
    async def _generate_share_fields(
        self, message_text: str, body: dict, __request__, __user__, __model__
    ) -> dict[str, str]:
        if __request__ is None or not self.valves.use_llm_share_generation:
            return {}
        generation_model = self.valves.share_model.strip() or self._resolve_model_id(
            body, __model__
        )
        ...
        payload = {
            ...
            "max_tokens": max(
                self.valves.share_generation_max_tokens,
                MIN_SHARE_GENERATION_MAX_TOKENS,
            ),
            "messages": [
                {"role": "system", "content": self.valves.share_generation_prompt.strip()},
                {
                    "role": "user",
                    "content": (
                        "Assistant message to convert into an OpenWebUI Community "
                        f"post draft:\n\n{message_text}"
                    ),
                },
            ],
        ...
        try:
            response = await generate_chat_completion(
                __request__, payload, user_model, bypass_system_prompt=True
            )
            return self._extract_share_content_from_text(
                self._extract_chat_completion_text(response)
            )
        except Exception as exc:
            self._debug_log(
                "LLM Community share generation failed; falling back to normalized content",
                error=str(exc),
            )
            return {}

    def _truncate_value(self, text: str, limit: int) -> tuple[str, int, bool]:
        original_length = len(text)
        truncated = original_length > limit
        return text[:limit].strip(), original_length, truncated

    async def _resolve_draft(
        self, message_text: str, body: dict, __request__, __user__, __model__
    ) -> CommunityShareDraft:
        markdown_message = self._normalize_markdown_content(message_text)
        generated = await self._generate_share_fields(
            message_text, body, __request__, __user__, __model__
        )
        if not self.valves.use_llm_share_generation:
            generated = {}

        title_source = "llm_generated" if generated.get("title") else "derived_first_line"
        content_source = (
            "llm_markdown"
            if generated.get("content")
            else "assistant_message_markdown"
        )

        derived_title = self._normalize_title(markdown_message) or self.valves.default_title.strip()
        title_value = self._normalize_title(generated.get("title", "")) or derived_title
        title_value, _, title_truncated = self._truncate_value(
            title_value, self.valves.max_title_chars
        )

        content_value = self._normalize_markdown_content(
            generated.get("content", "") or markdown_message
        )
        content_value, original_length, content_truncated = self._truncate_value(
            content_value, self.valves.max_content_chars
        )

        return CommunityShareDraft(
            title=title_value,
    ...
    def _build_share_url(self, draft: CommunityShareDraft) -> str:
        query_params = {
            key: value
            for key, value in {
                "title": draft.title,
        ...
        base_url = self.valves.community_post_base_url.strip() or "https://openwebui.com/post"
        query = urlencode(query_params, quote_via=quote)
        return f"{base_url}?{query}" if query else base_url

    def _build_execute_code(self, share_url: str, return_result: bool) -> str:
        popup_success = (
            "return { ok: true, method: 'popup_window', popupBlocked: false, "
            "newTabBlocked: false, url };"
            if return_result
            else "return;"
        )
        tab_success = (
            "return { ok: true, method: 'new_tab', popupBlocked: true, "
            "newTabBlocked: false, url };"
            if return_result
            else "return;"
        )
        blocked_result = (
            "return { ok: false, method: 'blocked', popupBlocked: true, "
            "newTabBlocked: true, url };"
            if return_result
            else "return;"
        )
        return "".join(
            (
                ...
                "try {",
                f"  popupWindow = window.open(url, '{COMMUNITY_POPUP_NAME}', '{COMMUNITY_POPUP_FEATURES}');",
                "} catch (e) {}",
    ...
    async def _open_share_dialog(
        self, share_url: str, __event_call__, __event_emitter__
    ) -> dict[str, Any]:
        if __event_call__ is not None:
            result = await __event_call__(
                {
                    "type": "execute",
                    "data": {"code": self._build_execute_code(share_url, True)},
                }
            )
            ...
            await __event_emitter__(
                {
                    ...
                    "data": {
                        "code": (
                            f"(() => {{ {self._build_execute_code(share_url, False)} }})();"
                        )
                    },
                }
            )
        ...
        raise ValueError(
            "No browser execute channel is available. __event_call__ and "
            "__event_emitter__ are both missing."
        )

    async def _handle_loading(self, __event_emitter__) -> None:
        await self._emit_notification(
            __event_emitter__, "info", self.valves.loading_notification_text
        )

    async def _handle_success(self, __event_emitter__) -> None:
        await self._emit_notification(
            __event_emitter__, "success", self.valves.success_notification_text
        )

    async def _handle_failure(self, exc: Exception, __event_emitter__) -> None:
        if self.valves.debug:
            LOGGER.exception("[share_to_openwebui_community] Action failed")
        else:
            LOGGER.error("[share_to_openwebui_community] Action failed: %s", exc)
    ...
    async def action(
        ...
        try:
            ...
            if not any((draft.title, draft.content)):
                raise ValueError("No shareable OpenWebUI Community content could be prepared.")
            await self._handle_loading(__event_emitter__)
            share_url = self._build_share_url(draft)
            execute_result = await self._open_share_dialog(
                share_url, __event_call__, __event_emitter__
            )
            self._debug_log(
                "Prepared OpenWebUI Community share draft",
                title_source=draft.title_source,
                content_source=draft.content_source,
                title_length=len(draft.title),
                original_content_length=draft.original_content_length,
                final_content_length=len(draft.content),
